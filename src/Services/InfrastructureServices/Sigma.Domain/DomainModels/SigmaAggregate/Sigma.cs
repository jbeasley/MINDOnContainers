...
namespace MINDOnContainers.Services.InfrastructureServices.Sigma.Domain.DomainModels.SigmaAggregate
{
    public class Sigma : Entity, IAggregateRoot
    {
        ...
        public Sigma(List<Device> devices, List<Plane> planes, List<Location> locations) : this()
        {
            _devices = devices;
        ...
        Location location, Plane plane = null, int? tenantId = null)
        {
            ...
            var devices = this._devices.Where(device => 
                device.Location == location && device.Status == DeviceStatus.Production);
        ...
        public Location GetLocation(string locationName) => this.Locations.Where(location => location.Name == locationName).SingleOrDefault();
    }
}
