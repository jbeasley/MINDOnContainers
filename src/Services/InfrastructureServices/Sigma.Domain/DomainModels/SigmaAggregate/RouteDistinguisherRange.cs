...
namespace MINDOnContainers.Services.InfrastructureServices.Sigma.Domain.DomainModels.SigmaAggregate
{
    public class RouteDistinguisherRange : ValueObject
    {
        ...
        public RouteDistinguisherRange(string name, int administratorSubField, int assignedNumberSubFieldStartValue, int assignedNumberSubFieldEndValue) : this()
        {
            ...
            {
                throw new SigmaDomainException($"The administrator subfield value for route distinguisher range '{name}' must be between 1 and 65535.");
            }

            if (assignedNumberSubFieldStartValue < 1 || assignedNumberSubFieldStartValue > (2 ^ 32 - 1))
            {
                throw new SigmaDomainException($"The start value for the assigned number subfield of route distinguisher range '{name}' must be between 1 and 4294967295.");
            }

            if (assignedNumberSubFieldEndValue < 2 || assignedNumberSubFieldEndValue > (2 ^ 32))
            {
                throw new SigmaDomainException($"The end value for the assigned number subfield of route distinguisher range '{name}' must be between 2 and 4294967296.");
            }

            if (assignedNumberSubFieldStartValue >= assignedNumberSubFieldEndValue)
            {
                throw new SigmaDomainException($"The start value for the assigned number subfield of route distinguishe range '{name}' must be less than the end value.");
            }
